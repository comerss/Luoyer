...
public class BaseViewHolder extends RecyclerView.ViewHolder {
    ...
    public BaseViewHolder setImageUrl(int viewId, String imageUrl) {
        ImageView view = getView(viewId);
        Glide.with(convertView.getContext()).load(imageUrl).crossFade().into(view);
        return this;
    }

    public BaseViewHolder setImageUrl(int viewId, String imageUrl, int defResourceId) {
        ImageView view = getView(viewId);
        Glide.with(convertView.getContext()).load(imageUrl).crossFade().placeholder(defResourceId).into(view);
        return this;
    }

    public BaseViewHolder setImageUrl(int viewId, String imageUrl, int defResourceId, BitmapTransformation... transformations) {
        ImageView view = getView(viewId);
        Glide.with(convertView.getContext()).load(imageUrl).crossFade().placeholder(defResourceId).transform(transformations).into(view);
        return this;
    }
    ...
    /**
     * 为textview设置Icon_font
     */
    public <T extends View> T getIconFont(int viewId,Typeface iconTypeface) {
        View view = views.get(viewId);
        if (view == null) {
            view = convertView.findViewById(viewId);
            ((TextView)view).setTypeface(iconTypeface);
            views.put(viewId, view);
        }
        return (T) view;
    }
