...
import com.qiezzi.choseviewlibrary.adapter.WheelAdapter;
...
public class AddressView extends View {
    ...
    // 修改这个值可以改变滑行速度
    private static final int VELOCITYFLING = 1;
    ...
    private void initLoopView(Context context) {
        ...
        gestureDetector = new GestureDetector(context, new LoopViewsGestureListener(this));
    ...
    void smoothScroll(ACTION action) {
        ...
        mFuture = mExecutor.scheduleWithFixedDelay(new SmoothsScrollTimerTask(this, mOffset), 0, 10, TimeUnit.MILLISECONDS);
    }

    protected final void scrollBy(float velocityY) {
        cancelFuture();

        mFuture = mExecutor.scheduleWithFixedDelay(new InertiasTimerTask(this, velocityY), 0, VELOCITYFLING, TimeUnit.MILLISECONDS);
    }
    ...
    public final void setTextSize(float size) {
        if (size > 0.0F && !customTextSize) {
            textSize = (int) (context.getResources().getDisplayMetrics().density * size+0.5f);
            paintOuterText.setTextSize(textSize);
            paintCenterText.setTextSize(textSize);
        }
    }
    ...
    protected final void onItemSelected() {
        if (onItemSelectedListener != null) {
            postDelayed(new OnItemsSelectedRunnable(this), 200L);
        }
    }
